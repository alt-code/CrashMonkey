...
function analyzeTestCases(filepath, fileContents) {
    ...
    Infer.withContext(new Infer.Context(defs, null), function() {
        ...
        rootVars = _.keys(rootVars);
        
        // Correlating which file a required module was pointing to 
        retObj.rootVarsModules = {};
        var modules = CodeParser.getModulesForReqs(fileContents, rootVars);
        _.each(rootVars, function (key, i) {
            var md = modules[i];
            var curr = /(.*)\/[^\/]+/.exec(filepath)[1];
            
            //Go up a directory for every ..
            while(/^\.\./.test(md)) {
                curr = /(.*)\/[^\/]+/.exec(curr)[1];
                md = /^\.\.\/(.*)/.exec(md)[1];
            }
            ...
            retObj.rootVarsModules[key] = curr + "/" + md;
        });
        
        parseSuite(tests, rootVars, filename, fileContents);
        retObj.tests = tests;
    });
