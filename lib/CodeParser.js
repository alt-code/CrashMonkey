...
function getGlobalReqs(fileContents) {
    // Only does it for the pattern var abc = require("something")
    // TODO for the pattern var abc = require("something").that;
...
function getModulesForReqs(fileContents, reqs) {
    // Only does it for the pattern var abc = require("something")
    // TODO for the pattern var abc = require("something").that;
    var modules = [];
    _.each(reqs, function (req) {
        req = req.replace(/\$/g, "\\$");
        var regx = new RegExp(req + "\\s*=\\s*require\\(['\"]([^'\"]+)['\"]\\)");
        var module = regx.exec(fileContents);
        if (module)
...
module.exports.getModulesForReqs = getModulesForReqs;