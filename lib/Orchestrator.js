...
function generateTestCases(repo, opts) {
    ...
    if (opts.tests) {
        for (var i = 0; i < opts.tests.length; i++) {
            ...
            tcFile.rootVars = tc.rootVars;
            tcFile.rootVarsModules = tc.rootVarsModules;
            testcases.push(tcFile);
        }
        
        correlateFuncsToTestCases(files, testcases);
    }
...
function correlateFuncsToTestCases(files, testcases) {
    for (var i = 0; i < files.length; i++) {
        if (files[i].funcs.length === 0)
            continue;
        
        for (var j = 0; j < files[i].funcs.length; j++) {
            var calls = getCallsForFunc(testcases, files[i].name, files[i].funcs[j].name);
            files[i].funcs[j].tcanalysis = {};
            files[i].funcs[j].tcanalysis.functioncalls = calls;
        }
    }
}

function getCallsForFunc(tcFiles, filename, funcname) {
    var calls = [];
    filename = "./" + filename;
    ...
    for (var i = 0; i < tcFiles.length; i++) {
        ...
        var target = _.findKey(file.rootVarsModules, x => x === filename);
        calls = calls.concat(getCallsForTestSuite(file.tests, target, funcname));
    }
    
    return calls;
}
    
function getCallsForTestSuite(suite, target, funcname) {
    var calls = [];
    
    if (suite.type === "testcase") {
        for (var key in suite.varUses) {
            if (!suite.varUses.hasOwnProperty(key))
                continue;
            
            if (key === target) {
                var functioncalls = suite.varUses[key].functioncalls;
                
                for (var i = 0; i < functioncalls.length; i++) {
                    var prop = functioncalls[i].property;
                    
                    if (funcname === "." && prop.length === 0) {
                        calls.push(functioncalls[i]);
                    }
                    
                    //Needs to be extended to support non root functions
                }
            }
        }
    }
        ...
        for (var i = 0; i < suite.testcases.length; i++) {
            calls = calls.concat(getCallsForTestSuite(suite.testcases[i], target, funcname));
        }
    }
