    ...
    _.each(files, function(file) {
        ...
        _.each(file.funcs, function (func) {
            ...
            if (func.tcanalysis && func.tcanalysis.functioncalls.length > 0) {
                var paramPermutations = [];
                _.each(func.tcanalysis.functioncalls, function (call) {
                    var paramValues = [];
                    _.each(call.arguments, function (arg) {
                        paramValues.push(getInstancesForType(arg.inferredType));
                    });
